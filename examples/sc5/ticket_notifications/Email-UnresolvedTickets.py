...
def get_info(sc):
    now = datetime.now()
    # Set the minimum age for tickets. Here we are setting the value to a week ago.
    minticketage = (now + timedelta(days=-7))
    ...
    # In this situation, we are getting any ticket that is a week old or older.
    oldtickets = [item for item in assignedtickets if ((datetime.fromtimestamp(float(item['assignedTime']))) <= minticketage)]
	...
	ticketagedays = str((now - assignedtime).days)
...
def send_info(ticketdata):
	...
	# subject to use the appropriate language for each.
        if len(ticketsbyuser[email]) == 1:
            html += "<h3>REMINDER: The following ticket is assigned to you in SecurityCenter for resolution:</h3>"
        else:
            html += "<h3>REMINDER: The following tickets are assigned to you in SecurityCenter for resolution:</h3>"
	# Iterate over the ticket IDs for this user and build the HTML tables for each ticket.
        ...
        msg['Subject'] = "Reminder: Unresolved Tickets in SecurityCenter"
        msg['From'] = emailfrom
        msg['To'] = emailto
